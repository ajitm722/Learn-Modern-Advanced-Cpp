...
int main() {
	cout << Max<double>(7.3, 2.6) << endl;  // Expands template to:
	
	/*
	double Max(const double& t1, const double& t2) {       // The arguments and return value automatically get assigned this type
    if (t1 > t2)
        return t1; 
    return t2;
	}
